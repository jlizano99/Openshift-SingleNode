#!/bin/bash
### Modificado por Juan Jose Lizano - jlizano@gbm.net
...
if [ "$INTERACTIVE" = "true" ]; then
	read -rp "You have an available secondary block device?: (Y/N) " choice;
...
sed -i -e "s/^enabled=1/enabled=0/" /etc/yum.repos.d/ansible.repo
